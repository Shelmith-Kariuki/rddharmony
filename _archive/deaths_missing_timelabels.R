## This script runs DDharmonize_validate_DeathCounts() and reports if there are time labels present in the
...
## Run the code
for (i in 41: length(locids)){

  clean_df <- DDharmonize_validate_DeathCounts(locid = locids[i],
                                               times = c(1950, 2021),
                                               process = c("census", "vr"),
  ...
  if(!is.null(clean_df) & length(missing_timelabs) > 0){
    sheet_append(ss = ss,  missing_data, sheet = "Deaths" )
  }
  missing_timelabs <- 0
  missing_data <- tibble()
}
...
## shelmith@shelmith-Precision-5510:~/Documents/Github/rddharmony/_archive$ Rscript deaths_missing_timelabels.R
