...
vignette: >
  %\VignetteIndexEntry{DDharmonize Vitals5}
...
This function takes five-year vital counts from census and registers from the UNDP database and standardizes/harmonizes them. It is a very crucial process of the harmonization process. It is defined as follows:

> DDharmonize_Vitals5(indata, type = c("births","deaths"))[^1]

To show how this function works, we shall use the `vitals5_df` dataset that is embedded on this package. This dataset represents births by age of mother and sex of child in Norway for the year 2002 and for cases where SexID == "Both sexes".
...
```{r}
new_df <- vitals5_df %>% 
  select(LocID, LocName, DataSourceYear,DataStatusName, SexName, SexID,DataSourceYear, TimeLabel, TimeMid, starts_with("Age"), DataValue, SeriesID) %>% select(-agesort) %>% 
  arrange(AgeLabel)
...
The function should ideally be run over each of the specific SexIDs (Males (1), Females (2) and Both sexes (3)), but when it is applied on births data, only "Both sexes" counts are considered. It is also implemented only in cases where we have at least one abridged age group.
...
We begin by subsetting the data to ensure that only have "Both sexes" data and none of the data values are missing. 
...
We have situations where we the "Unknown" value represents the difference between the reported total and the summation of the data values (computed totals) and in such a case, we also set the "Unknown" to 0 [^5].
...
We then reconcile early age groups for births, for example in this case, we have 0-14 and since the minimum age should be 10, we set this to 10-14 [^6]. We also reconcile ages 0-1, 1-4 and 0-4 such that if 0-4 is missing and 0-1 and 1-4 are present, then sum both to get 0-4 [^7].
...
We proceed to check whether there are multiple open age groups (TRUE/FALSE Output), and if so, compute closed age groups from multiple open age groups and add to data if missing [^8].
...
Finally, a note is created to alert the user about missing data and this process is repeated for all the sex ids (in the case of deaths data).
